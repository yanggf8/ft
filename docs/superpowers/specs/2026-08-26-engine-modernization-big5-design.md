...
> **⚠️ 本節三選一已於 2026-09-03 被取代**，見 `2026-09-03-f4-f5-if-then-design-note.md` §5.4（裁決 N10）。
> 原因:「沒發生」混合了「情境沒出現」（無資料）與「情境出現但反應不同」（一筆否證）兩種相反證據，
> 使下方 §F8「編碼方式須事前寫死」這條要求在三選一的形狀下**無法被滿足**。
> 改為兩段式:先問情境有無（`absent` / `occurred`），再問反應（`hit` / `miss` / `other`）；
> 計入規則與 F8 分母規則見該備註 §5.4.1，**提問順序約束**（第 1 段須在 forecast 揭露前）見 §5.4.2。
> 實作以該備註為準;本節保留原文供審查歷史對帳。
...
- **判定規則（上線前預先登記，事後不得更改）**：
  ...
  - 「沒發生」的編碼方式在資料蒐集前寫死，不得事後決定
    （**已滿足**:2026-09-03 定案於 `2026-09-03-f4-f5-if-then-design-note.md` §5.4.1；
    F8 分母一律為 `occurred`，且兩組第 1 段問法須逐字相同）
